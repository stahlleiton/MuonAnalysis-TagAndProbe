...
void TagProbeFitter::saveFitPlot(RooWorkspace* w){
  ...
  TCanvas canvas2("pull_canvas");
  canvas.Divide(2,2);
  canvas2.Divide(2,2);
  vector<RooPlot*> frames, frames2;
  ...
  TH1F *hPvlu = new TH1F("hPvlu","",3,0, 3);
  TH1F *hPullPass = new TH1F("hPullPass","",massBins, minMass, maxMass);
  TH1F *hPullFail = new TH1F("hPullFail","",massBins, minMass, maxMass);
  TH1F *hPullAll = new TH1F("hPullAll","",massBins, minMass, maxMass);
  hChi2->Sumw2();
  hPvlu->Sumw2();
  hPullPass->Sumw2();
  hPullFail->Sumw2();
  ...
  //pdf.plotOn(frames[0], Slice(efficiencyCategory, "Passed"), ProjWData(*dataPass), LineColor(kGreen+1));
  pdf.plotOn(frames[0], Slice(efficiencyCategory, "Passed"), ProjWData(*dataPass), LineColor(kGreen+1), Components("backgroundPass"), LineStyle(kDashed));
  pdf.plotOn(frames[0], Slice(efficiencyCategory, "Passed"), ProjWData(*dataPass), LineColor(kGreen+1));
  frames[0]->Draw();
  // dmoon
  ...
  double Chi2 = 0;
  int nFullBinsPull = 0;
  double *ypullpass = hpullpass->GetY();

  for (unsigned int i=lowSgn; i < highSgn; i++) {
      if (hdatapass->GetBinContent(i) == 0) continue;
      nFullBinsPull++;
      //cout<<"ypullpass["<<i<<"] : "<<ypullpass[i]<<endl;
      Chi2 = Chi2 + pow(ypullpass[i],2);
      hPullPass->SetBinContent(i+1,ypullpass[i]);
  }
  double UnNormChi2 = Chi2;
  int nFitParam = fitRes->floatParsFinal().getSize();
  int Dof = nFullBinsPull - nFitParam;

  TLatex t; t.SetNDC();
  char latexchi2[512] = {0};
  sprintf(latexchi2,"#chi^{2}/dof = %0.1f/%d",UnNormChi2,Dof);
  t.DrawLatex(0.62,0.82,latexchi2);

  double pVlu = TMath::Prob(UnNormChi2,Dof);
  char ltPvlu[512];
  sprintf(ltPvlu,"p-value : %0.4f", pVlu); 
  t.DrawLatex(0.62,0.76,ltPvlu);
  hChi2->SetBinContent(1, UnNormChi2/Dof);
  ...
  UnNormChi2 = 0.0;
  nFullBinsPull = 0;
  double *ypullfail = hpullfail->GetY();

  for (unsigned int i=lowSgn; i < highSgn; i++) {
      if (hdatafail->GetBinContent(i) == 0) continue;
      nFullBinsPull++;
      //cout<<"ypullfail["<<i<<"] : "<<ypullfail[i]<<endl;
      Chi2 = Chi2 + pow(ypullfail[i],2);
      hPullFail->SetBinContent(i+1,ypullfail[i]);
  }
  UnNormChi2 = Chi2;
  Dof = nFullBinsPull - nFitParam;

  sprintf(latexchi2,"#chi^{2}/dof = %0.1f/%d",UnNormChi2,Dof);
  t.DrawLatex(0.62,0.82,latexchi2);

  pVlu = TMath::Prob(UnNormChi2,Dof);
  sprintf(ltPvlu,"p-value : %0.4f", pVlu); 
  t.DrawLatex(0.62,0.76,ltPvlu);
  hChi2->SetBinContent(2, UnNormChi2/Dof);
  ...
  UnNormChi2 = 0.0;
  nFullBinsPull = 0;
  double *ypullall = hpullall->GetY();

  for (unsigned int i=lowSgn; i < highSgn; i++) {
      if (hdataall->GetBinContent(i) == 0) continue;
      nFullBinsPull++;
      //cout<<"ypullall["<<i<<"] : "<<ypullall[i]<<endl;
      Chi2 = Chi2 + pow(ypullall[i],2);
      hPullAll->SetBinContent(i+1,ypullall[i]);
  }
  UnNormChi2 = Chi2;
  Dof = nFullBinsPull - nFitParam;
  ...
  sprintf(ltPvlu,"p-value : %0.4f", pVlu); 
  t.DrawLatex(0.62,0.76,ltPvlu);
  hChi2->SetBinContent(3, UnNormChi2/Dof);
